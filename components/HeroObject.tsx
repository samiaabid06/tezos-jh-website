...
export default function HeroObject() {
  ...
  useEffect(() => {
    const t = setTimeout(() => setMaterialized(true), 200);
    return () => clearTimeout(t);
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Particle field + energy beams to center */}
      <div
      ...
      {/* Orbiting tech tags */}
      <motion.div
        className="absolute w-[92%] h-[92%] pointer-events-none"
        animate={{ rotate: 360 }}
        ...
        {orbitTags.map((tag, i) => (
          ...
          >
            <motion.div
              ...
              style={{ transform: `rotate(-${tag.angle}deg)` }}
              animate={{
                boxShadow: [
      ...
      <motion.div
        ...
        animate={materialized ? { scale: 1, opacity: 1 } : {}}
        ...
        >
          {[
            ...
            >
              {face.content && (
                <span
                  className="text-4xl font-bold text-tezos-gradient"
                  style={{ fontFamily: "var(--font-space-grotesk)" }}
                >
                  ꜩ
                </span>
              )}
              {/* Scan sweep light across each face */}
              <motion.div
      ...
      {/* Pulse rings */}
      <motion.div
        className="absolute w-40 h-40 sm:w-56 sm:h-56 lg:w-72 lg:h-72 rounded-full border border-[#00E5FF]/40 pointer-events-none"
        animate={{ scale: [1, 1.5, 1], opacity: [0.6, 0, 0.6] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeOut" }}
      />
      <motion.div
        className="absolute w-40 h-40 sm:w-56 sm:h-56 lg:w-72 lg:h-72 rounded-full border border-[#2C7DF7]/30 pointer-events-none"
      ...
      {/* Chromatic-aberration glitch flicker on hover */}
      {hovering && (
        <motion.div
          className="absolute w-40 h-40 sm:w-56 sm:h-56 lg:w-72 lg:h-72 pointer-events-none mix-blend-screen"
          animate={{ x: [0, 2, -2, 1, 0], opacity: [0, 0.4, 0, 0.3, 0] }}
          transition={{ duration: 0.25, repeat: Infinity, repeatDelay: 1.4 }}
          style={{
            background:
